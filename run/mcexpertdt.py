...
from stainer.cyclegan import module
import stainer.cyclegan.tf2lib as tl
import stainer.cyclegan.pylib as misc
...
class MCExpertDT:
    ...
    def __init__(self,
        ...
        self.stain_transfer = stain_transfer
        ...
        if self.stain_transfer:
            self.prepare_stainer()

    def prepare_stainer(self):

        self.logger.info(">>> Preparing Stain Transfer Module ...")
        self.G_S = module.ResnetGenerator(input_shape=(self.stain_info['crop_size'], self.stain_info['crop_size'], 3))
        tl.Checkpoint(checkpoint_kwargs={"G_A2B": self.G_S},
                        directory=misc.join(self.stain_info['stain_dir'], '_weights', self.stain_info['stain_id'], 'checkpoints')).restore().expect_partial()           
    ...
    def apply_cyclegan(self, batch):
        return self.G_S(batch, training=False)
    ...
    def predict_root(self):
        ...
        if self.check_mc('Root'):
            pred_len = self.model_params['Root']['params']['iter']
        else:
            pred_len = 1

        self.root_preds_array = np.zeros((self.root_array.shape[0], self.root_array.shape[1], pred_len, 2))
        self.root_uncert_array = np.zeros_like(self.root_array, dtype=float)

        image_batch = []
        patch_indices = []

        mc_uncert = Uncertainty()
        mc_uncert.set_metric(metric_params=self.model_params['Root']['params'])
        ...
        def process_batch(batch, indices):
            ...
            if self.stain_transfer:
                batch = tf.convert_to_tensor(np.stack([np.array(img, dtype=np.float32) for img in batch]), dtype=tf.float32)
                batch = tf.image.resize(batch, (self.stain_info['crop_size'], self.stain_info['crop_size']), method=tf.image.ResizeMethod.BILINEAR)
                batch = [np.array(region, dtype=np.float32) for region in batch]
                batch = tf.clip_by_value(batch, 0, 255) / 127.5 - 1
                batch = self.apply_cyclegan(batch)
                batch = (batch + 1) / 2
                batch = tf.image.resize(batch, (self.model_crop_size, self.model_crop_size), method=tf.image.ResizeMethod.BILINEAR)
                batch = tf.clip_by_value(batch, 0, 1)

            else:
                batch = [region.resize((self.model_crop_size, self.model_crop_size)) for region in batch]
                batch = [np.clip(np.array(region), 0, 255)/255.0 for region in batch]
                batch = tf.convert_to_tensor(np.array(batch))

            predictions, pred_classes, pred_probs = vgg16.predict(batch)
            for pred, pred_class, pred_prob, index in zip(predictions, pred_classes, pred_probs, indices):
                ...
                uncertainty = mc_uncert.get_uncertainty(pred)
                if mc_uncert.is_certain():
                    assigned_label = pred_class+1
                else:
                    assigned_label = pred_class+6
                self.root_array[y, x] = assigned_label
                self.root_preds_array[y, x, :, :] = pred
                self.root_prob_array[y, x] = pred_prob
                self.root_uncert_array[y, x] = uncertainty
    ...
    def predict_node(self):
        ...
        if not self.check_stage_requirement(stage="Node"):
            self.logger.warning(f"--------> No Root Analysis Found ...")
            return
        ...
        if self.check_mc('Node'):
            pred_len = self.model_params['Node']['params']['iter']
        else:
            pred_len = 1
        
        self.node_preds_array = np.zeros((self.node_array.shape[0], self.node_array.shape[1], pred_len, 2))
        self.node_uncert_array = np.zeros_like(self.node_array, dtype=float)
        ...
        mc_uncert = Uncertainty()
        mc_uncert.set_metric(metric_params=self.model_params['Node']['params'])
        ...
        def process_batch(batch, info_batch):
            ...
            for pred, pred_class, pred_prob, info in zip(predictions, pred_classes, pred_probs, info_batch):
                ...
                uncertainty = mc_uncert.get_uncertainty(pred)
                if mc_uncert.is_certain():
                    assigned_label = node_label
                else:
                    assigned_label = 7

                self.node_array[y, x] = assigned_label
                self.node_preds_array[y, x, :, :] = pred
                self.node_prob_array[y, x] = pred_prob
                self.node_uncert_array[y, x] = uncertainty
        ...
        self.node_cert = 0
        prune_threshold = self.model_params['Node']['prune_threshold']
        ...
        if self.node_count_1+self.node_count_2 != 0:
            ...
            if prune_threshold is None or str(prune_threshold) == 'None':
                self.stages_status['Node'] = 'Pruned'
                self.logger.info("--------> No Threshold Has Been Set, Node is Pruned! Connecting Root Directly to Leaves ...")

            elif isinstance(prune_threshold, (int, float)) and self.node_cert >= prune_threshold:
                ...
                self.stages_status['Node'] = 'Pruned'
                self.logger.info("--------> Node is Pruned! Connecting Root Directly to Leaves ...")
        else:
            self.stages_status['Node'] = 'Pruned'
            self.logger.info("--------> Node is Pruned! Connecting Root Directly to Leaves ...")
    ...
    def predict_leaf1(self):
        ...
        if self.check_mc('Leaf1'):
            pred_len = self.model_params['Leaf1']['params']['iter']
        else:
            pred_len = 1
        
        self.leaf1_preds_array = np.zeros((self.leaf1_array.shape[0], self.leaf1_array.shape[1], pred_len, 2))
        self.leaf1_uncert_array = np.zeros_like(self.leaf1_array, dtype=float)
        ...
        mc_uncert = Uncertainty()
        mc_uncert.set_metric(metric_params=self.model_params['Leaf1']['params'])
        ...
        def process_batch(batch, info_batch):
            ...
            for pred, pred_class, pred_prob, info in zip(predictions, pred_classes, pred_probs, info_batch):
                ...
                uncertainty = mc_uncert.get_uncertainty(pred)
                if mc_uncert.is_certain():
                    assigned_label = leaf1_label
                else:
                    assigned_label = 7

                self.leaf1_array[y, x] = assigned_label
                self.leaf1_preds_array[y, x, :, :] = pred
                self.leaf1_prob_array[y, x] = pred_prob
                self.leaf1_uncert_array[y, x] = uncertainty
    ...
    def predict_leaf2(self):
        ...
        if self.check_mc('Leaf2'):
            pred_len = self.model_params['Leaf2']['params']['iter']
        else:
            pred_len = 1
        
        self.leaf2_preds_array = np.zeros((self.leaf2_array.shape[0], self.leaf2_array.shape[1], pred_len, 2))
        self.leaf2_uncert_array = np.zeros_like(self.leaf2_array, dtype=float)
        ...
        mc_uncert = Uncertainty()
        mc_uncert.set_metric(metric_params=self.model_params['Leaf2']['params'])
        ...
        def process_batch(batch, info_batch):
            ...
            for pred, pred_class, pred_prob, info in zip(predictions, pred_classes, pred_probs, info_batch):
                ...
                uncertainty = mc_uncert.get_uncertainty(pred)
                if mc_uncert.is_certain():
                    assigned_label = leaf2_label
                else:
                    assigned_label = 7

                self.leaf2_array[y, x] = assigned_label
                self.leaf2_preds_array[y, x, :, :] = pred
                self.leaf2_prob_array[y, x] = pred_prob
                self.leaf2_uncert_array[y, x] = uncertainty
    ...
    def predict_leaf(self):
        ...
        if self.stages_status['Node'] == 'Pruned':
            ...
            self.subtype_counts['Uncertain (NT)'] = int(value_counts_leaf.get(6, 0)/2)
            self.subtype_counts['Uncertain (T)'] = int(value_counts_leaf.get(7, 0)/2)
            ...
            self.subtype_counts['Uncertain (NT)'] = int(self.value_counts_leaf1.get(6, 0))
            self.subtype_counts['Uncertain (T)'] = int(self.value_counts_leaf1.get(7, 0))
            ...
            self.subtype_counts['Uncertain (NT)'] = int(self.value_counts_leaf2.get(6, 0))
            self.subtype_counts['Uncertain (T)'] = int(self.value_counts_leaf2.get(7, 0))
    ...
    def save_tree(self, save_dir):
        ...
        tree_info['prune_threshold'] = self.model_params['Node']['prune_threshold']
        tree_info['node_cert'] = self.node_cert

        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        uncerts_dir = os.path.join(save_dir, "uncerts")

        if not os.path.exists(uncerts_dir):
            os.makedirs(uncerts_dir)
        ...
        np.savez_compressed(
            ...
            array=self.root_refined_array,
            preds=self.root_preds_array
        )

        np.savez_compressed(
            os.path.join(uncerts_dir, "Root.npz"),
            uncert=self.root_uncert_array,
            probs=self.root_prob_array
        )

        np.savez_compressed(
            os.path.join(save_dir, "Node.npz"),
            array=self.node_refined_array,
            preds=self.node_preds_array
        )

        np.savez_compressed(
            os.path.join(uncerts_dir, "Node.npz"),
            uncert=self.node_uncert_array,
            probs=self.node_prob_array
        )

        if self.stages_status["Node"] == "Pruned":
            np.savez_compressed(
                os.path.join(save_dir, "Leaf1.npz"),
                array=self.leaf1_refined_array,
                preds=self.leaf1_preds_array
            )
            np.savez_compressed(
                os.path.join(uncerts_dir, "Leaf1.npz"),
                uncert=self.leaf1_uncert_array,
                probs=self.leaf1_prob_array
            )

            np.savez_compressed(
                os.path.join(save_dir, "Leaf2.npz"),
                array=self.leaf2_refined_array,
                preds=self.leaf2_preds_array
            )
            np.savez_compressed(
                os.path.join(uncerts_dir, "Leaf2.npz"),
                uncert=self.leaf2_uncert_array,
                probs=self.leaf2_prob_array
            )
    ...
    def get_metrics(self, metrics):
        ...
        for stage in stages_metrics:
            ...
            if stage=="Root":
                arrays = [self.root_refined_array, self.root_prob_array, self.root_uncert_array]
                remove_values = [0.0]

            elif stage=="Node":
                arrays = [self.node_refined_array, self.node_prob_array, self.node_uncert_array]
                remove_values = [0.0, 1.0]

            elif stage=="Leaf1":
                arrays = [self.leaf1_refined_array, self.leaf1_prob_array, self.leaf1_uncert_array]
                remove_values = [0.0, 1.0]

            elif stage=="Leaf2":
                arrays = [self.leaf2_refined_array, self.leaf2_prob_array, self.leaf2_uncert_array]
                remove_values = [0.0, 1.0]
            ...
            for label in unique_labels:
                ...
                uncert_for_class = arrays[2][indices]
                probs_by_class[label] = list(filter(lambda x: x != 0.0, probs_for_class.tolist()))
                uncert_by_class[label] = list(filter(lambda x: x != 0.0, uncert_for_class.tolist()))

            for value in remove_values:
                removed_labels = probs_by_class.pop(value, 'Key not found')
                removed_labels = uncert_by_class.pop(value, 'Key not found')

            del removed_labels

            probs_by_class = {stage_kwargs['keymap_prob'].get(k, k): v for k, v in probs_by_class.items()}
            uncert_by_class = {stage_kwargs['keymap_prob'].get(k, k): v for k, v in uncert_by_class.items()}
            ...
            for class_name in probs_by_class.keys():
                for metric in metrics:
                    ...
                    metrics_dict[f'{stage}_{class_name}_{metric}_uncert'] = get_stats(type=metric, input=uncert_by_class[class_name])
    ...
    def save_figs(self, save_dir, id, slidename, prob_plot='hist'):
        ...
        if self.save_plots:
            ...
            slide_dir = os.path.join(save_dir, str(id), slidename)
    ...
    def save_plot_stage(self, stage, name_dir, dpi=300, alpha=1):
        ...
        if stage=="Root":
            ...
            uncert_array = self.root_uncert_array
            ...
            uncert_array = self.node_uncert_array
            ...
            uncert_array = self.leaf1_uncert_array
            ...
            uncert_array = self.leaf2_uncert_array
        ...
        fig, axes = plt.subplots(1, len(arrays)+1, figsize=(30, 10))
        ...
        im = axes[2].imshow(uncert_array, cmap='Greys')
        axes[2].set_title('Stage Uncertainty')
        fig.colorbar(im, ax=axes[2], shrink=0.8, pad=0.05)
    ...
    def plot_stage_on_slide(self, stage, alpha=1):
        ...
        im = axes[2].imshow(uncert_array, cmap='gray')
        axes[2].set_title('Stage Uncertainty')
    ...
    def PlotStage(self, stage, alpha=1):
        ...
        self.plot_dict = {
            ...
            "Root": {
                ...
                "stage_colors": [self.colors_rgba['White'], self.colors_rgba['Pink'], self.colors_rgba['Dark Red'], "", "", "", self.colors_rgba['Dark Pink'], self.colors_rgba['Red']],
                "label_dict": {"0": "Background", "1": "Non-Tumor", "2": "Tumor", "6": "Uncertain (NT)", "7": "Uncertain (T)"},
                "keymap_prob":{1.0:"Non-Tumor", 2.0:"Tumor", 6.0:"Uncertain (NT)", 7.0:"Uncertain (T)"}
            },

            "Node": {
                "titles": ['Node', 'Refined Node'],
                "stage_colors": [self.colors_rgba['White'], self.colors_rgba['Pink'], self.colors_rgba['Blue'], self.colors_rgba['Orange'], "", "", self.colors_rgba['Dark Pink'], self.colors_rgba['Red']],
                "label_dict": {"0": "Background", "1": "Non-Tumor", "2": "CHROMO/ONCO", "3": "ccRCC/pRCC", "6": "Uncertain (NT)", "7": "Uncertain (T)"},
                "keymap_prob":{2.0:"CHROMO/ONCOCYTOMA", 3.0:"ccRCC/pRCC", 6.0:"Uncertain (NT)", 7.0:"Uncertain (T)"}

            },

            "Leaf1": {
                "titles": ['Leaf1', 'Refined Leaf1'],
                "stage_colors": [self.colors_rgba['White'], self.colors_rgba['Pink'], "", "", self.colors_rgba['Purple'], self.colors_rgba['Navy Blue'], self.colors_rgba['Dark Pink'], self.colors_rgba['Red']],
                "label_dict": {"0": "Background", "1": "Non-Tumor", "4": "CHROMO", "5": "ONCOCYTOMA", "6": "Uncertain (NT)", "7": "Uncertain (T)"},
                "keymap_prob":{4.0:"CHROMO", 5.0:"ONCOCYTOMA", 6.0:"Uncertain (NT)", 7.0:"Uncertain (T)"}
            },

            "Leaf2": {
                "titles": ['Leaf2', 'Refined Leaf2'],
                "stage_colors": [self.colors_rgba['White'], self.colors_rgba['Pink'], self.colors_rgba['Lime Green'], self.colors_rgba['Cyan'], "", "", self.colors_rgba['Dark Pink'], self.colors_rgba['Red']],
                "label_dict": {"0": "Background", "1": "Non-Tumor", "2": "ccRCC", "3": "pRCC", "6": "Uncertain (NT)", "7": "Uncertain (T)"},
